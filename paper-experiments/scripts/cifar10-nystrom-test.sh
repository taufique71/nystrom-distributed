	...
	do
		...
		do
			...
			if [ "$SYSTEM" == "perlmutter-cpu" ]; then
				...
				BIN=$HOME/Codes/nystrom-distributed/paper-experiments/build_cpu_cifar10/c_matmul/nystrom
			elif [ "$SYSTEM" == "perlmutter-gpu" ]; then
				PY=$HOME/Codes/nystrom-distributed/paper-experiments/nystrom.py
				BIN=$HOME/Codes/nystrom-distributed/paper-experiments/build_gpu_cifar10/c_matmul/nystrom
			elif [ "$SYSTEM" == "perlmutter-gpu-cpu" ]; then
				PY=$HOME/Codes/nystrom-distributed/paper-experiments/nystrom.py
				BIN=$HOME/Codes/nystrom-distributed/paper-experiments/build_cpu_cifar10/c_matmul/nystrom
			fi
			...
			for TRY in $(seq 1 $N_TRY); do
				#echo "Value: $TRY"
				STDOUT_FILE=$SCRATCH/nystrom/cifar10_nystrom/"$ALG"_"$IMPL"_"$SYSTEM"_"$N_NODE"_"$N_PROC"_"$THREAD_PER_PROC"_"$N"_"$R"_"$MATMUL1_P1"x"$MATMUL1_P2"x"$MATMUL1_P3"_"$MATMUL2_P1"x"$MATMUL2_P2"x"$MATMUL2_P3.$TRY"
                AFILE=$SCRATCH/nystrom/dataset/cifar10-"$KERNEL".bin
                YFILE=$SCRATCH/nystrom/dataset/cifar10-"$KERNEL"-"cpu-$R-y".bin
                ZFILE=$SCRATCH/nystrom/dataset/cifar10-"$KERNEL"-"cpu-$R-z".bin
				...
				if [ "$IMPL" == "cpp" ]; then
					srun -N $N_NODE -n $N_PROC -c $THREAD_PER_PROC --ntasks-per-node=$PROC_PER_NODE --cpu-bind=cores \
                        ...
                        -afile $AFILE -yfile $YFILE -zfile $ZFILE \
						&> $STDOUT_FILE
					#srun -N $N_NODE -n $N_PROC -c $THREAD_PER_PROC --ntasks-per-node=$PROC_PER_NODE --cpu-bind=cores \
