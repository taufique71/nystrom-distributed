...
#pragma omp parallel
    ...
    // double* contribZred = NULL;
#ifdef USE_CUBLAS
    CUDA_CHECK( cudaMalloc(reinterpret_cast<void **>(&contribZ), sizeof(double) * (r * r) ) );
    // CUDA_CHECK( cudaMalloc(reinterpret_cast<void **>(&contribZred), sizeof(double) * (r * r) ) );
#else
	contribZ = new double[r*r];
	// contribZred = new double[r*r];
#endif
	...
	//ParMat Z(r, r, grid, 'B'); // B face for column split distrib of Z
	{
        ...
        // Benchmarking comparison: allreduce vs reduce-scatter timing (not part of algorithm)
        // #ifdef BARRIER
        // 		MPI_Barrier(MPI_COMM_WORLD);
        // #endif
        // 		t0 = MPI_Wtime();
        // 		MPI_Allreduce(contribZ, contribZred, r*r, MPI_DOUBLE, MPI_SUM, grid2.colWorld);
        // #ifdef BARRIER
        // 		MPI_Barrier(MPI_COMM_WORLD);
        // #endif
        // 		t1 = MPI_Wtime();
    ...
    // CUDA_CHECK(cudaFree(contribZred));
