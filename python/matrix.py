...
class ParMat:
    ...
    def generate_rand(self, seed, generator="xoroshiro"):
        ...
        prng.random(self.localMat.shape, dtype=self.localMat.dtype, out=self.localMat)
    ...
    def parallelReadBinary(self, path, world):
        # Collective MPI-IO read of a column-major (Fortran-order) binary file
        # into self.localMat. Each process reads its own subarray.
        lsizes = [self.nRowLocal, self.nColLocal]
        gsizes = [self.nRowGlobal, self.nColGlobal]
        starts = [self.localRowStart, self.localColStart]

        fh = MPI.File.Open(world, path, MPI.MODE_RDONLY)
        view = MPI.DOUBLE.Create_subarray(
            sizes=gsizes,
            subsizes=lsizes,
            starts=starts,
        ...
        fh.Read_all(self.localMat)
        fh.Close()
        view.Free()

    def parallelWriteBinary(self, path, world):
        # Collective MPI-IO write of self.localMat into a column-major
        # (Fortran-order) binary file. Each process writes its own subarray.
        lsizes = [self.nRowLocal, self.nColLocal]
        gsizes = [self.nRowGlobal, self.nColGlobal]
        starts = [self.localRowStart, self.localColStart]
...
def matmul1_comm(A, B, generator = 'xoroshiro'):
    ...
    # Gather local matrices of B along the grid columns
    t0 = MPI.Wtime()
    B.generate_rand(seed=B.grid.myrank, generator="xoroshiro")
    t1 = MPI.Wtime()
    if (B.grid.myrank == 0):
        print("Time to generate B:", t1 - t0, "sec")
